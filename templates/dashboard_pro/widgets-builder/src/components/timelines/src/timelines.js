...
import { event as currentEvent, mouse, namespace, namespaces, select } from 'd3-selection';
import { zoom as d3z } from 'd3-zoom'

//const event = require("d3-selection").event
//console.log(currentEvent, mouse, namespace, namespaces, select)
...
var timelines = function() {
		...
		var appendBackgroundBar = function (yAxisMapping, index, g, data, datum) {
			var greenbarYAxis = ((itemHeight + itemMargin) * yAxisMapping[index]) + margin.top - (itemHeight + itemMargin);
			g.selectAll("svg")
				.data(data).enter()
				...
				.attr("height", itemHeight + 2*itemVerticalPadding)
		...
		var appendLabel = function (gParent, yAxisMapping, index, hasLabel, datum) {
			...
			var rowsDown          = margin.top + (fullItemHeight/2) + fullItemHeight * (yAxisMapping[index] || 1)  - itemHeight;
			gParent.append("text")
		...
		function timelines (gParent) {
				...
				d.forEach( function(datum, index){
					...
					view.selectAll("svg")
						...
						.on("mouseover", function (d, i, e) {
							...
							mouseover(d, i, datum, getCursorXY(e));
						})
					...
					function getStackPosition(d, i) {
						if (stacked) {
							return margin.top + (itemHeight + itemMargin) * yAxisMapping[index] + itemVerticalPadding - (itemHeight + itemMargin);
						}
						return margin.top + itemVerticalPadding;
					}
					function getStackTextPosition(d, i) {
						if (stacked) {
							return margin.top + (itemHeight + itemMargin) * yAxisMapping[index] + itemHeight * 0.75 + itemVerticalPadding;
						}
						return margin.top + itemHeight * 0.75 + itemVerticalPadding;
					}
				});
			});
			...
			var timeAxisYPosition = showAxisTop ? aboveFirstItem : belowLastItem - itemHeight;
			var gX;
			...
			function setHeight() {
				if (!height && !gParentSize.height) {
					...
					if (!height) {
						...
						if(!gParentItem.node().height.baseVal.value) {
		...
		timelines.itemVerticalPadding = function (p) {
			if (!arguments.length) return itemVerticalPadding;
			itemVerticalPadding = p;
			return timelines;
		};
