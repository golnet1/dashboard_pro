    ...
    export default {
        ...
        computed: {
            style () {
                ...
                max = this.layout.reduce(function (prev, current) {
                            return prev.y > current.y ? prev : current;
                        });
                var height = (this.cellSize.h + this.margin) * (max.y + max.h) + "px"
                console.log(max, height)
                // update properties object
                style = { ...style, height: height, maxWidth: style.minWidth, maxHeight: style.minHeight }
        ...
        methods: {
            ...
            getPixelPositionById (id) {
                if (this.dragging.boxLayout && this.dragging.boxLayout.id === id) {
                    let pixels = utils.positionToPixels(this.dragging.boxLayout, this.cellSize, this.margin, this.outerMargin)
                ...
                if (this.resizing.boxLayout && this.resizing.boxLayout.id === id) {
                    let pixels = utils.positionToPixels(this.resizing.boxLayout, this.cellSize, this.margin, this.outerMargin)
                ...
                return utils.positionToPixels(boxLayout, this.cellSize, this.margin, this.outerMargin)
            },
            isBoxVisible (id) {
                var boxLayout = this.getBoxLayoutById(id)
            ...
            enableDragging (box) {
                ...
                let validateTargetPosition = (targetX, targetY) => {
                    if (targetX + this.dragging.boxLayout.w > this.maxColumnCount) {
                        targetX = this.maxColumnCount - this.dragging.boxLayout.w
                    } else {
                        if (targetX < 0) {
                            targetX = 0
                        }
                    }
                    if (targetY + this.dragging.boxLayout.h > this.maxRowCount) {
                        targetY = this.maxRowCount - this.dragging.boxLayout.h
                    } else {
                        if (targetY < 0) {
                            targetY = 0
                        }
                ...
                box.$on('dragUpdate', evt => {
                    ...
                    if (!utils.isFree(this.pinnedLayout, {
                        ...this.dragging.boxLayout,
                        x: this.dragging.boxLayout.x + moveBy.x,
                        y: this.dragging.boxLayout.y + moveBy.y
                    })) {
                        return
                    }

                    let { targetX, targetY } = validateTargetPosition(
                        this.dragging.boxLayout.x + moveBy.x,
                        this.dragging.boxLayout.y + moveBy.y
                    )

                    // check if box has moved
                    if (this.placeholder.x === targetX && this.placeholder.y === targetY) {
                        return
                    }
            ...
            enableResizing (box) {
                ...
                let validateTargetSize = (targetW, targetH) => {
                    if (this.resizing.boxLayout.x + targetW > this.maxColumnCount) {
                        targetW = this.maxColumnCount - this.resizing.boxLayout.x
                    } else {
                        if (targetW < 1) {
                            targetW = 1
                        }
                    }
                    if (this.resizing.boxLayout.y + targetH > this.maxRowCount) {
                        targetH = this.maxRowCount - this.resizing.boxLayout.y
                    } else {
                        if (targetH < 1) {
                            targetH = 1
                        }
                ...
                box.$on('resizeUpdate', evt => {
                    ...
                    if (!utils.isFree(this.pinnedLayout, {
                        ...this.resizing.boxLayout,
                        w: this.resizing.boxLayout.w + resizeBy.x,
                        h: this.resizing.boxLayout.h + resizeBy.y
                    })) {
                        return
                    }

                    let { targetW, targetH } = validateTargetSize(
                        this.resizing.boxLayout.w + resizeBy.x,
                        this.resizing.boxLayout.h + resizeBy.y
                    )

                    // check if box size has changed
                    if (this.placeholder.w === targetW && this.placeholder.h === targetH) {
                        return
                    }
                    this.placeholder = utils.updateBoxPosition(this.placeholder, {
                        w: targetW,
