              ...
              <v-tab v-for="(item, index) in widget.series" :key="item.key">
                ...
                <v-tab-item v-for="item in widget.series" :key="item.key">
                  ...
                  <v-col class="py-0">
                    ...
                    <v-col class="py-0">
                      <v-switch v-model="item.fill" :label="$t('widget.graph.fill')"></v-switch>
                    </v-col>
                    <v-col class="py-0">
                      <v-switch v-model="item.steppedLine" :label="$t('widget.graph.step_line')"></v-switch>
                    </v-col>
                    <v-col class="py-0">
                      <v-select
                        v-model="item.scale"
  ...
  methods: {
    ...
    async fillData() {
      ...
      for (let index = 0; index < this.widget.series.length; index++) {
        ...
        var yID = element.scale == "last" && yAxes.length > 0 ? yAxes[yAxes.length-1].id: element.key
        if (element.scale)
          dataset["yAxisID"] = yID
        datasets.push(dataset);
        if (!(element.scale == "last" && yAxes.length > 0))
          yAxes.push({
            id: yID,
            type: "linear",
